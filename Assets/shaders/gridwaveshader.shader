...
Shader "PDT Shaders/gridwaveshader" {
	Properties {
		...
		_waveamount("wave", float) = 0.1
		_speedwave("speedwave", float) = 0.1
	...
	SubShader {
		...
		{
			v.vertex.y+=sin(abs(v.texcoord.x*2-1)*30+_Time.y*_speedwave)*_waveamount;
